...
int pca::CPriceCalculator::GetTestPrice() { return 8; }
...
void pca::CPriceCalculator::AddToProduct_CreateConsumptionOption(
    std::string sProduct, std::string sOption) {
  if (nullptr == m_upReality) {
    std::cout << "pca::CPriceCalculator::AddToProduct_CreateConsumptionOption "
                 "needs a m_upReality before being called"
              << std::endl;
  }
  ...
  if (pProductRef) {
    ...
    std::cout << "Product: " << sProduct
              << " not found in "
                 "CPriceCalculator::AddToProduct_CreateConsumptionOption"
              << std::endl;
  }
}
...
void pca::CPriceCalculator::AddToPerson_SetProductAmount(std::string sPerson,
                                                         std::string sProduct,
                                                         double dAmount) {
...
void pca::CPriceCalculator::AddToPerson_SetSatisfactionCurveForOption(
    std::string sPerson, std::string sOption, double dValueAt0,
    double dMaxValue) {
  if (nullptr == m_upReality->GetLastMarketRef())
    return;
...
void pca::CPriceCalculator::
    AddToPerson_SetSatisfactionCurveForComplementaryCombo(
        std::string sPerson, std::string sComplementaryCombo, double dValueAt0,
        double dMaxValue) {
  if (nullptr == m_upReality->GetLastMarketRef())
    return;
...
void pca::CPriceCalculator::
    AddToPerson_SetSatisfactionCurveForSupplementaryCombo(
        std::string sPerson, std::string sSupplementaryCombo, double dValueAt0,
        double dMaxValue) {
  if (nullptr == m_upReality->GetLastMarketRef())
    return;
...
double pca::CPriceCalculator::GetProductAmount(std::string sProductName,
                                               std::string sPerson) {
  CProduct *pProductRef = m_upReality->GetProduct(sProductName);
  CPerson *pPersonRef = m_upReality->GetLastMarketRef()->GetPersonRef(sPerson);
...
double pca::CPriceCalculator::GetOptionAmount(std::string sOptionName,
                                              std::string sPerson) {
  COption *pOptionRef = m_upReality->GetOption(sOptionName);
  CPerson *pPersonRef = m_upReality->GetLastMarketRef()->GetPersonRef(sPerson);
...
double pca::CPriceCalculator::GetTradedAmount(std::string sProductName,
                                              std::string sPerson) {
  CProduct *pProductRef = m_upReality->GetProduct(sProductName);
  CPerson *pPersonRef = m_upReality->GetLastMarketRef()->GetPersonRef(sPerson);
...
double pca::CPriceCalculator::GetDesiredProdAmount(std::string sPerson,
...
void pca::CPriceCalculator::PrintPersonOptionAdjustmentToFile(
    std::string sPerson) {
  CPerson *pPersonRef = m_upReality->GetLastMarketRef()->GetPersonRef(sPerson);
...
void pca::CPriceCalculator::AddOptionToComplCombo(std::string sComplCombo,
                                                  std::string sOption) {
  if (nullptr == m_upReality) {
...
void pca::CPriceCalculator::CreateSupplCombo(std::string sSupplCombo) {
  if (nullptr == m_upReality) {
    std::cout << "pca::CPriceCalculator::sSupplCombo needs a m_upReality "
                 "before being called"
              << std::endl;
  }
...
void pca::CPriceCalculator::AddOptionToSupplCombo(std::string sSupplCombo,
                                                  std::string sOption,
                                                  double dWeight) {
  if (nullptr == m_upReality) {
